...
let numberOfWeeks = Math.floor((Date.now() - severalTimeAgo) / oneWeek)
...
function getData(url1, url2, url3, url4, url5){
    ...
    $.ajax({
        ...
        $('#allMeet0').html(msg[0][0] + msg[1][0] + msg[2][0] + msg[3][0] + msg[4][0])
        $('#allMeet1').html(msg[0][1] + msg[1][1] + msg[2][1] + msg[3][1] + msg[4][1])
        $('#allMeet2').html(msg[0][2] + msg[1][2] + msg[2][2] + msg[3][2] + msg[4][2])
        $('#allMeet3').html(msg[0][3] + msg[1][3] + msg[2][3] + msg[3][3] + msg[4][3])
    }).fail(function( jqXHR, textStatus ) {
